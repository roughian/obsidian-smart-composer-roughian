...
- 저장 위치(Vault folder / CMDS Eagle (sync)): 설정 `imageGeneration.destination`(`src/core/image/image-destination.ts`). sync는 커뮤니티 플러그인 `cmds-eagle`(id 고정, 포크 `cmds-eagle-vian`은 사용 안 함)의 `settings.imagePasteBehavior`(eagle/local/cloud/ask)를 따른다. 전달은 `src/core/image/image-delivery.ts`가 `eagle-bridge.ts`로 `uploadImageToEagle`/`getActiveCloudProvider().upload`를 호출하고 성공 시 볼트 사본을 휴지통으로 보낸다. 실패하면 볼트 사본 유지 + `artifact.metadata.destinationError`로 채팅 경고. ask면 요청 시 `ImageDestinationModal`.
