> [!IMPORTANT]
> **This is the roughian fork** of [Smart Composer](https://github.com/glowingjade/obsidian-smart-composer) by **Heesu Suh (glowingjade)**, redistributed under the original MIT license.
> The original project, its documentation and its author's credit remain as in the upstream README below. This fork is maintained independently by [roughian](https://github.com/roughian); please do not report fork-specific issues upstream.
>
> **What this fork changes**
> - Plan (subscription) model catalog kept current: Claude Fable 5.1 / Opus 5 / Sonnet 5, GPT-6 Astra and GPT-5.6 family, with per-model reasoning-effort rules (`src/core/llm/openaiPlanModels.ts`, `claudePlanModels.ts`).
> - Image workflow: clipboard/drag-and-drop images are resized and re-encoded before upload; GPT image generation shows live progress in the chat and the finished image replaces it in place; Apply opens the diff view (Accept Incoming / Current / Both).
> - Image storage destination: keep in the vault or sync with the [CMDS Eagle](https://github.com/johnfkoo951/cmds-eagle) plugin (Eagle / cloud / ask), delegating to that plugin's own settings.
> - Image-to-image: pasted images are passed to the image model as references.
> - Vision safeguards, per-model image-model selection fix, and restored CSS tokens/animations.
> - `MODEL_UPDATE_GUIDE.md` documents how to add new Plan models and where the image features live.
...
> **What's New**
>
> **v1.3.0** — GPT-5.6 Sol/Terra/Luna, Claude Fable 5/Opus 5/Sonnet 5, inline edit, prompt and image queues, image generation, and improved vault retrieval
>
...
This build requires an explicit, versioned consent in Smart Composer settings before a Claude Plan token can be used. OpenAI Plan uses the Codex subscription route. Gemini Plan remains visible only as unavailable because its consumer OAuth path is no longer active.
...
- **Current Plan models**: GPT-5.6 Sol, Terra, and Luna; Claude Fable 5, Opus 5, Sonnet 5, and Opus 4.8 compatibility
- **Inline Edit**: Generate and review a focused replacement or insertion directly in the editor
- **Prompt Queue**: Queue, edit, send, resume, or cancel prompts while a response is running
- **Image Generation Queue**: Generate one image at a time, save it to the vault, preview it, and insert an embed
- **Retrieval Modes**: Choose automatic, embedding, or Plan reranking, with focused or exhaustive folder reads
...
### Updating Plan models

Plan models are defined in `src/core/llm/planModelCatalog.ts`. Add or update a catalog entry there, include any old selected-model ID in `LEGACY_PLAN_MODEL_ALIASES`, then update provider-specific capability checks and tests. The settings loader merges this catalog idempotently, so existing enable and reasoning choices are retained while new models appear automatically.
