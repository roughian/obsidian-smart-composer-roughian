...
export function ChatModelsSubSection({
  ...
  const handleDeleteChatModel = async (modelId: string) => {
    if (
      ...
      new Notice('Cannot remove a model selected by an active feature')
      return
    }
  ...
  const handleToggleEnableChatModel = async (
    ...
    if (
      ...
      new Notice('Cannot disable a model selected by an active feature')
  ...
  return (
    <div>
      ...
      <div className="smtcmp-settings-desc">
        Models used for chat, Apply, inline editing, and image generation
      </div>
          ...
          <tbody>
            {settings.chatModels.map((chatModel) => (
              <tr key={chatModel.id}>
                ...
                <td>
                  {chatModel.providerType === 'gemini-plan' ? (
                    <span className="mod-muted">Unavailable</span>
                  ) : (
                    <ObsidianToggle
